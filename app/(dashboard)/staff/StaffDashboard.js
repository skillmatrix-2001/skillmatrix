    ...
    <div style={{ minHeight: '100vh', background: 'var(--background)' }}>
      <style>{`
        ...
        @media (max-width: 767px) {
          ...
          .staff-card-col { border-right: none !important; border-bottom: 1px solid var(--border); }
          .staff-card-col:last-child { border-bottom: none; }
        }
        .student-card { transition: all 0.2s ease; cursor: pointer; }
        .student-card:hover { box-shadow: 0 0 0 1px var(--primary-soft-border); background: var(--surface-2) !important; }
        .cert-item, .proj-item {
          background: var(--background); border: 1px solid var(--border); border-radius: 8px; padding: 12px; transition: all 0.15s;
        }
        .cert-item:hover, .proj-item:hover { border-color: var(--card-hover-border); background: var(--surface-2); }
        .batch-highlight {
          background: var(--primary-soft); border: 1px solid var(--primary-soft-border); border-radius: 20px;
          padding: 4px 12px; font-weight: 600; color: var(--primary); display: inline-block; font-size: 13px; margin-top: 8px;
        }
      `}</style>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '2rem 1rem' }}>
        ...
        <div style={{ marginBottom: '2rem', display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: '1rem' }}>
          <div>
            <h1 style={{ color: 'var(--text-primary)', fontSize: 28, fontWeight: 700, marginBottom: 8 }}>
              Staff Dashboard {staffUser.department && `– ${staffUser.department}`}
            </h1>
            <p style={{ color: 'var(--text-dim)', fontSize: 14 }}>View students and achievements in your department</p>
          </div>
          <button
            ...
            onMouseOver={(e) => { if (displayedCount > 0) e.target.style.background = 'var(--btn-primary-hover-bg)'; }}
            onMouseOut={(e) => { if (displayedCount > 0) e.target.style.background = 'var(--btn-primary-bg)'; }}
          >
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1rem', marginBottom: '2rem' }}>
          {[
            { label: showStaffAccounts ? 'Staff' : 'Students', value: displayedCount },
            { label: 'Batch Years', value: allBatchYears.length },
            { label: 'Semester Filter', value: selectedSemester === 'all' ? 'All' : `Sem ${selectedSemester}` },
            { label: showStaffAccounts ? 'Department' : 'Total Posts', value: showStaffAccounts ? staffUser.department : totalPosts },
          ].map((stat, idx) => (
            <div key={idx} style={cardStyle}>
              <p style={{ color: 'var(--text-dim)', fontSize: 12, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 8 }}>
                {stat.label}
              </p>
        ...
        <div style={{ ...cardStyle, marginBottom: '1.5rem' }}>
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              <button
                ...
                style={{
                  ...
                  background: showStaffAccounts ? 'var(--primary-soft)' : 'var(--btn-ghost-bg)',
                  borderColor: showStaffAccounts ? 'var(--primary-soft-border)' : 'var(--btn-ghost-border)',
                }}
              >
          ...
          {!showStaffAccounts && (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '1rem', marginTop: '1rem' }}>
              <div>
                <label style={{ display: 'block', color: 'var(--text-dim)', fontSize: 12, marginBottom: 6 }}>Batch Year</label>
              ...
              <div>
                <label style={{ display: 'block', color: 'var(--text-dim)', fontSize: 12, marginBottom: 6 }}>Semester</label>
              ...
              <div>
                <label style={{ display: 'block', color: 'var(--text-dim)', fontSize: 12, marginBottom: 6 }}>Certificate Category</label>
          ...
          {showAdvancedFilters && !showStaffAccounts && (
            <div style={{ marginTop: '1rem', paddingTop: '1rem', borderTop: '1px solid var(--border)' }}>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '1rem' }}>
                <div>
                  <label style={{ display: 'block', color: 'var(--text-dim)', fontSize: 12, marginBottom: 6 }}>Date From</label>
                  <input type="date" value={dateFrom} onChange={(e) => setDateFrom(e.target.value)} style={selectStyle} />
                </div>
                <div>
                  <label style={{ display: 'block', color: 'var(--text-dim)', fontSize: 12, marginBottom: 6 }}>Date To</label>
                  <input type="date" value={dateTo} onChange={(e) => setDateTo(e.target.value)} style={selectStyle} />
                </div>
                <div>
                  <label style={{ display: 'block', color: 'var(--text-dim)', fontSize: 12, marginBottom: 6 }}>Reg No. From</label>
                  <div style={{ display: 'flex', alignItems: 'center', background: 'var(--input-bg)', border: '1px solid var(--input-border)', borderRadius: 8, overflow: 'hidden' }}>
                    <span style={{ padding: '10px 0 10px 14px', color: 'var(--text-dim)', fontSize: 14, background: 'transparent', whiteSpace: 'nowrap' }}>9513</span>
                    <input
                      type="text"
                ...
                <div>
                  <label style={{ display: 'block', color: 'var(--text-dim)', fontSize: 12, marginBottom: 6 }}>Reg No. To</label>
                  <div style={{ display: 'flex', alignItems: 'center', background: 'var(--input-bg)', border: '1px solid var(--input-border)', borderRadius: 8, overflow: 'hidden' }}>
                    <span style={{ padding: '10px 0 10px 14px', color: 'var(--text-dim)', fontSize: 14, background: 'transparent', whiteSpace: 'nowrap' }}>9513</span>
                    <input
                      type="text"
        ...
        {(searchQuery || selectedBatch !== 'all' || selectedSemester !== 'all' || selectedCategory !== 'all' || dateFrom || dateTo || regFrom || regTo) && (
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: '1.5rem' }}>
            <span style={{ color: 'var(--text-dim)', fontSize: 13 }}>Active filters:</span>
        ...
        <div style={{ marginTop: 32, display: 'flex', justifyContent: 'space-between', alignItems: 'center', color: 'var(--text-dim)', fontSize: 13 }}>
          ...
          <button onClick={showStaffAccounts ? fetchStaffAccounts : fetchFilteredStudents} disabled={loading} style={{ background: 'none', border: 'none', color: 'var(--text-secondary)', cursor: 'pointer', fontSize: 13 }} onMouseOver={(e) => e.target.style.color = 'var(--text-primary)'} onMouseOut={(e) => e.target.style.color = 'var(--text-secondary)'}>
            {loading ? 'Refreshing...' : 'Refresh Data'}
          </button>
        </div>

        {/* Instructions */}
        <div style={{ marginTop: 32, background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: 12, padding: '1.5rem' }}>
          <h3 style={{ color: 'var(--text-primary)', fontSize: 14, fontWeight: 600, marginBottom: 8 }}>Staff Instructions</h3>
          <ul style={{ color: 'var(--text-secondary)', fontSize: 13, lineHeight: 1.6, listStyle: 'disc', paddingLeft: '1.25rem', margin: 0 }}>
            <li>You see only {showStaffAccounts ? 'staff accounts' : 'students'} from your department: <strong style={{ color: 'var(--text-primary)' }}>{staffUser.department || 'your department'}</strong></li>
...
const FilterTag = ({ label, onClear }) => (
  <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: 999, padding: '4px 10px', fontSize: 12, color: 'var(--text-secondary)' }}>
    {label}
    <button onClick={onClear} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)', fontSize: 14, padding: '0 2px' }}>×</button>
  </span>
);
  ...
  <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
    {[1,2,3].map(i => (
      <div key={i} style={{ background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: 12, padding: 18 }}>
        <div style={{ width: '25%', height: 24, background: 'var(--background)', borderRadius: 4, marginBottom: 16 }}></div>
        <div style={{ width: '75%', height: 16, background: 'var(--background)', borderRadius: 4, marginBottom: 8 }}></div>
        <div style={{ width: '50%', height: 16, background: 'var(--background)', borderRadius: 4 }}></div>
      </div>
    ))}
  </div>
);
...
const EmptyState = ({ onReset }) => (
  <div style={{ textAlign: 'center', padding: '3rem', background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: 12 }}>
    ...
    <h3 style={{ color: 'var(--text-primary)', fontSize: 18, fontWeight: 600, marginBottom: 8 }}>No results found</h3>
    <p style={{ color: 'var(--text-dim)', fontSize: 14, marginBottom: 24 }}>Try adjusting your filters</p>
    <button onClick={onReset} style={{ background: 'var(--btn-primary-bg)', color: 'var(--btn-primary-text)', border: 'none', borderRadius: 8, padding: '10px 16px', fontSize: 14, fontWeight: 500, cursor: 'pointer' }}>Reset Filters</button>
  </div>
);
...
const StudentList = ({ students, showProjectsColumn, onViewProfile }) => (
  <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
    {students.map((student) => {
      ...
      return (
        <div key={student._id} className="student-card" onClick={() => onViewProfile(student.registerNumber)} style={{ background: 'var(--surface-1)', border: '1px solid var(--border)', borderRadius: 12, overflow: 'hidden' }}>
          <div className="staff-card-row">
            <div className="staff-card-col" style={{ padding: '1.25rem', borderRight: '1px solid var(--border)', background: 'var(--primary-soft)' }}>
              <h3 style={{ color: 'var(--text-primary)', fontWeight: 700, fontSize: 16, margin: '0 0 4px 0' }}>{student.name}</h3>
              <p style={{ color: 'var(--text-secondary)', fontSize: 13, fontFamily: 'monospace', margin: '0 0 12px 0' }}>{student.registerNumber}</p>
              <span className="batch-highlight">Batch: {student.batchYear ? `${student.batchYear} – ${Number(student.batchYear) + 4}` : '—'}</span>
            </div>
            <div className="staff-card-col" style={{ padding: '1.25rem', borderRight: showProjectsColumn ? '1px solid var(--border)' : 'none' }}>
              <h4 style={{ color: 'var(--text-dim)', fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.08em', margin: '0 0 12px 0' }}>Certificates ({certs.length})</h4>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {certs.length === 0 ? (
                  <p style={{ color: 'var(--text-dim)', fontSize: 12, fontStyle: 'italic', margin: 0 }}>No certificates</p>
                ) : (
                  certs.slice(0, 3).map(cert => (
                    <div key={cert._id} className="cert-item">
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 4 }}>
                        <span style={{ color: 'var(--text-primary)', fontWeight: 600, fontSize: 13 }}>{cert.title}</span>
                        <span style={{ background: 'var(--primary-soft)', border: '1px solid var(--primary-soft-border)', color: 'var(--primary)', padding: '2px 8px', borderRadius: 12, fontSize: 10, fontWeight: 500 }}>Sem {cert.semester || '—'}</span>
                      </div>
                      {cert.issuedBy && <p style={{ color: 'var(--text-secondary)', fontSize: 11, margin: '4px 0 0 0' }}>Issued by: {cert.issuedBy}</p>}
                    </div>
                  ))
                )}
                {certs.length > 3 && <p style={{ color: 'var(--text-dim)', fontSize: 11, margin: 0 }}>+{certs.length - 3} more</p>}
              </div>
            </div>
            {showProjectsColumn && (
              <div className="staff-card-col" style={{ padding: '1.25rem' }}>
                <h4 style={{ color: 'var(--text-dim)', fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.08em', margin: '0 0 12px 0' }}>Projects ({projs.length})</h4>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  {projs.length === 0 ? (
                    <p style={{ color: 'var(--text-dim)', fontSize: 12, fontStyle: 'italic', margin: 0 }}>No projects</p>
                  ) : (
                    projs.slice(0, 3).map(proj => (
                      <div key={proj._id} className="proj-item">
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                          <span style={{ color: 'var(--text-primary)', fontWeight: 600, fontSize: 13 }}>{proj.title}</span>
                          <span style={{ background: 'var(--success-soft)', border: '1px solid var(--success-soft-border)', color: 'var(--success)', padding: '2px 8px', borderRadius: 12, fontSize: 10, fontWeight: 500 }}>Sem {proj.semester || '—'}</span>
                        </div>
                      </div>
                    ))
                  )}
                  {projs.length > 3 && <p style={{ color: 'var(--text-dim)', fontSize: 11, margin: 0 }}>+{projs.length - 3} more</p>}
                </div>
              </div>
            )}
          </div>
          <div style={{ padding: '0.5rem 1.25rem', borderTop: '1px solid var(--border)', background: 'var(--surface-2)', display: 'flex', justifyContent: 'flex-end' }}>
            <span style={{ color: 'var(--primary)', fontSize: 12, fontWeight: 500 }}>Click to view full profile →</span>
          </div>
        </div>
      );
    })}
...
const StaffList = ({ staff, onViewProfile }) => (
  <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
    {staff.map((s) => (
      <div key={s._id} className="student-card" onClick={() => onViewProfile(s.staffId)} style={{ background: 'var(--surface-1)', border: '1px solid var(--border)', borderRadius: 12, overflow: 'hidden' }}>
        <div style={{ padding: '1.25rem' }}>
          <h3 style={{ color: 'var(--text-primary)', fontWeight: 700, fontSize: 16, margin: '0 0 4px 0' }}>{s.name}</h3>
          <p style={{ color: 'var(--text-secondary)', fontSize: 13, fontFamily: 'monospace', margin: '0 0 8px 0' }}>Staff ID: {s.staffId}</p>
          <p style={{ color: 'var(--text-dim)', fontSize: 12 }}>Department: {s.department}</p>
        </div>
        <div style={{ padding: '0.5rem 1.25rem', borderTop: '1px solid var(--border)', background: 'var(--surface-2)', display: 'flex', justifyContent: 'flex-end' }}>
          <span style={{ color: 'var(--primary)', fontSize: 12, fontWeight: 500 }}>Click to view profile and certificates →</span>
        </div>
      </div>
    ))}
  </div>
