...
export default async function generateDOCX(data) {
  ...
  // ── LEFT COLUMN: Contact › Skills › Education › Academic ────────────
  const left = [];
  ...
  // ─── NEW: ACADEMIC PERFORMANCE (added to left column) ─────────────
  const hasAcademic = data.tenthPercentage || data.twelfthPercentage || data.cgpa;
  if (hasAcademic) {
    left.push(sHead("Academic"));
    if (data.tenthPercentage) left.push(bodyPara(`10th: ${data.tenthPercentage}%`, { after: 30 }));
    if (data.twelfthPercentage) left.push(bodyPara(`12th: ${data.twelfthPercentage}%`, { after: 30 }));
    if (data.cgpa) {
      const cgpaText = `CGPA: ${data.cgpa}` + 
        (data.cgpaSemester && data.cgpaSemester < 8 ? ` (upto ${data.cgpaSemester}${data.cgpaSemester === 1 ? 'st' : data.cgpaSemester === 2 ? 'nd' : data.cgpaSemester === 3 ? 'rd' : 'th'} semester)` : '');
      left.push(bodyPara(cgpaText, { after: 30 }));
    }
    left.push(spacer(60));
  }
